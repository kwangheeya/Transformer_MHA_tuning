...
class MultiHeadedAttention(nn.Module):
    ...
    def __init__(self, head_count, model_dim, dropout=0.1,
        ...
        self.linear_values = nn.Sequential(nn.Linear(model_dim, model_dim),
                                          nn.Tanh(),
                                          self.dp_v,
                                          nn.Linear(model_dim, model_dim),
                                          nn.Tanh(),
                                          self.dp_v,
                                          nn.Linear(model_dim, head_count * self.dim_per_head),
                                          nn.Tanh(),
                                          self.dp_v)
        '''
        self.linear_values = nn.Sequential(nn.Linear(model_dim, model_dim),
        ...
        if layer_cache is not None:
            if type == "self":
                query, key, value = self.linear_query(query),\
                                    ...
                                    self.linear_values(query) * query
                key = shape(key)
                value = shape(value)
